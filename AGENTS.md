...
state/               runtime records and signals; gitignored
  ...
  <id>.home-retire    the pending removal obligation a secondmate teardown recorded before releasing the home's treehouse lease, so an interrupted or refused slot removal is never mistaken for a finished retirement; written only by bin/fm-teardown.sh, retried and reported by bin/fm-bootstrap.sh's retired-home sweep as HOME_RETIRE lines, removed once the returned directory is provably gone
