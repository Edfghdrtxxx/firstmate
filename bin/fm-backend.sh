...
# empty|pending|pending-unproven|busy|unknown for callers that need a pre-submit
...
fm_backend_composer_state() {  # <backend> <target> [expected-label] -> empty|pending|pending-unproven|busy|unknown
  local backend=$1
  shift
  fm_backend_source "$backend" || { printf 'unknown'; return 0; }
  case "$backend" in
