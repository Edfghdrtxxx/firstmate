...
if [ "${1:-}" = "--key" ]; then
  ...
  *--resolve-key*)
  ...
  if [ "$INBOX_PLANE" = 1 ]; then
    ...
    case "$ring_rc" in
    1) echo "fm-send: doorbell skipped (composer visibly holds pending text or live reviewer work); the steer is durably recorded at $INBOX_RECORD and the watcher will re-ring" >&2 ;;
