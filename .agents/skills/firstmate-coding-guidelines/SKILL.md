...
- Prefer end-to-end regression coverage for a behavior change: drive the shipped `bin/` entry points as real processes against a fixture home, along the path an operator or firstmate actually takes.
  Do not add new unit tests by default, such as a case that sources a helper library and calls one function in isolation.
  Add one only when an end-to-end test cannot observe the failure, or when the requester explicitly asks for unit coverage.
