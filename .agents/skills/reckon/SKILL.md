...
Audit as an adversarial reviewer, not the doer: every "done" is a claim to disprove, and contradictions between records are the quarry ("X says landed — does Y agree?").

## Procedure

1. **Traverse the backlog.** `bin/fm-tasks-axi.sh list` — every task this session created or touched, plus every still-open record regardless of origin session (old in-flight tasks, open decisions, armed checks from dead sessions count too).
...
6. **Second pass.** Re-scan for categories the first pass never examined; attest each category checked, even when empty. An ambiguous, corrupt, or unreachable record is `unverified` — report it as open, never silently dropped or assumed done.
7. **Report.** One compact list: closed items with their artifacts, then open items with the exact blocker or next step, then leftovers. Give every open item a disposition: fix now, leave, or defer with reason. State plainly when nothing is missing; never pad the open list with non-issues.
